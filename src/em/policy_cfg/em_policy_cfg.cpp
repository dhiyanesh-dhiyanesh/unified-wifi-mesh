...
int em_policy_cfg_t::handle_1905_ack(unsigned char *buff, unsigned int len)
{
    ...
    {
	    if (get_state() == em_state_ctrl_set_policy_pending && response_msg_id == m_policy_req_msg_id) {
		    match_found = true;
		    m_policy_req_msg_id = 0;
		    break;
	    }
    }
    ...
    if (match_found) {
	    ...
	    em_printfout("No radio waiting for policy config ack for givin msg_id\n");
    }
    em_radios.clear();
    return 0;
}
